...
/** android.app.Activity用户在其中选择要应用于图像的过滤器。 */
class FilterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ActivityFilterBinding.inflate(layoutInflater).run {
            ...
            /**
             * 这里的数组集合, 返回的是then这个函数, 执行的次数
             */
            //检查我们是否有输出。
            viewModel.workInfo.observe(this@FilterActivity) { info ->
                Log.e("info.size", info.size.toString())
    ...
    private fun bindViews(binding: ActivityFilterBinding) {
        with(binding) {
            ...
            if (!multipleDestinationsPossible) {
                destinationsGroup.visibility = View.VISIBLE
            }
    ...
    private fun onStateChange(info: WorkInfo, binding: ActivityFilterBinding) {
        ...
        val a = info.outputData.getString(Constants.KEY_IMAGE_URI)  //获取数据
        Log.v("Constants.KEY_IMAGE_URI", a.toString())
